...
\begin{proof}
  For any $z:C$ we have
  \begin{align*}
    \modal(\fib{g\circ f}{z})
    & \eqvsym
      \modal(\sm{p:\fib{g}{z}}\fib{f}{\proj1(p)}) \\
    & \eqvsym
      \modal(\sm{p:\fib{g}{z}}\modal(\fib{f}{\proj1(p)})) \\
    & \eqvsym
      \modal(\sm{p:\fib{g}{z}}\unit) \\
    & \eqvsym
      \modal\fib{g}{z}.
  \end{align*}
...
\begin{proof}
  ...
  But given $b:B$, we have a map $\proj1 : \fib{f}{b} \to A$, hence $\modal\proj1 : \modal \fib{f}{b} \to \modal A$.
  And $\modal \fib{f}{b}$ is contractible since $f$ is $\modal$-connected, so it has a point $c_b$, and we define $g(\modalunit[B](b)) = \modal \proj1(c_b)$.
...
\begin{align*}
  \prd{b:B} P(b) & \eqvsym \prd{b:B} \Parens{\modal{\fib{f}b} \to P(b)}
  \tag{since $\modal{\fib{f}b}$ is contractible}\\
  & \eqvsym \prd{b:B} \Parens{\fib{f}b\to P(b)}
...
$P(b)\defeq \modal{\fib{f}b}$.
Then~\ref{item:conntest3} yields a map $c:\prd{b:B} \modal{\fib{f}b}$ with
$c(f(a))=\modalunit{\pairr{a,\refl{f(a)}}}$. To show that each $\modal{\fib{f}b}$ is contractible, we will show that $c(b)=w$ for any $b:B$ and $w:\modal{\fib{f}b}$.
In other words, we must show that the identity function $\modal{\fib{f}b} \to \modal{\fib{f}b}$ is equal to the constant function at $c(b)$.
By the universal property of $\modal{\fib{f}b}$, it suffices to show that they become equal when precomposed with $\modalunit[\fib{f}b]$, i.e.\ we may assume that $w = \modalunit\pairr{a,p}$ for some $a:A$ and $p:f(a)=b$.
...
{\allowdisplaybreaks
\begin{align*}
\modal{\fib{\varphi}{\pairr{b,v}}} & \eqvsym \modal{\sm{a:A}{u:P(a)}{p:f(a)= b} \trans{p}{g_a(u)}= v}\\
& \eqvsym \modal{\sm{w:\fib{f}b}{u:P(\proj1(w))} g_{\proj 1 w}(u)= \trans{\opp{\proj2(w)}}{v}}\\
& \eqvsym \modal{\sm{w:\fib{f}b} \fib{g(\proj1 w)}{\trans{\opp{\proj 2(w)}}{v}}}\\
& \eqvsym \modal{\sm{w:\fib{f}b} \modal{\fib{g(\proj1 w)}{\trans{\opp{\proj 2(w)}}{v}}}}\\
& \eqvsym \modal{\fib{f}b}
\end{align*}}%
...
Therefore, if either of $\modal{\fib{\varphi}{\pairr{b,v}}}$ or $\modal{\fib{f}b}$ is contractible, so is the other.

In particular, if $f$ is $\modal$-connected, then $\modal{\fib{f}b}$ is contractible for all $b:B$, and hence so is $\modal{\fib{\varphi}{\pairr{b,v}}}$ for all $(b,v):\sm{b:B} Q(b)$.
On the other hand, if $\varphi$ is $\modal$-connected, then $\modal{\fib{\varphi}{\pairr{b,v}}}$ is contractible for all $(b,v)$, hence so is $\modal{\fib{f}b}$ for any $b:B$ such that there exists some $v:Q(b)$.
...
\begin{proof}
We have
$\fib{\total f}{\pairr{x,v}}\eqvsym\fib{f(x)}v$
for each $x:A$ and $v:Q(x)$. Hence $\modal{\fib{\total f}{\pairr{x,v}}}$ is contractible if and only if
$\modal{\fib{f(x)}v}$ is contractible.
\end{proof}
...
Y \arrow[d,swap,"f"] \arrow[rr,"{\lam{y}\pairr{\fib{f}{f(y)},\blank,\pairr{y,\refl{f(y)}}}}"] & & \pointed{(\UU_P)} \arrow[r] \arrow[d] & \pointed{\UU} \arrow[d] \\
