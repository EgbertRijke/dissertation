...
The $n$-equivalences are easily described: they are just the maps that induce isomorphisms on the first $n$ homotopy groups. In this section our goal is to characterize the formally \'etale maps for the $n$-truncations. We call those maps \define{formally $n$-\'etale}. Note that the case of propositional truncation has already been addressed in \cref{eg:etale_prop}.
...
\begin{equation*}
\begin{tikzcd}
A' \arrow[d,swap,"{i'}"] \arrow[r,"g"] & A \arrow[d,"i"] \\
B' \arrow[r,swap,"f"] & B
\end{tikzcd}
\end{equation*}
where $i:A\to B$ is an $\modal'$-equivalence. Then $i':A'\to B'$ is a $\modal$-equivalence. 
\end{lem}

\begin{proof}
First we factor both $f$ and $g$, using the reflective factorization system of $\modal'$. This results in the diagram
\begin{equation*}
\begin{tikzcd}
A' \arrow[d,swap,"{i'}"] \arrow[r,"g_l"] & X \arrow[r,"g_r"] \arrow[d,swap,"h"] & A \arrow[d,"i"] \\
B' \arrow[r,swap,"f_l"] & Y \arrow[r,swap,"f_r"] & B
\end{tikzcd}
\end{equation*}
where the map $h$ is obtained by orthogonality. Then the square on the right is a pullback square (why?)
\end{proof}
\end{comment}
