\chapter{Reflexive coequalizers}\label{chap:rcoeq}
...
\end{align*}
\item Given a map $f:A\to X$, the \define{pre-kernel} $k(f)$ of $f$ is the reflexive relation on $A$ given by $x,y\mapsto f(x)=f(y)$.
\item The \define{indiscrete graph} $\nabla(X)$ on $X$ is the reflexive graph consisting of
\begin{align*}
\pts{\nabla(X)} & \defeq X \\
...
\item The \define{walking edge} $\mathcal{I}$ is an example of a reflexive graph. One way of defining it is by taking
\begin{align*}
\pts{\mathcal{I}} & \defeq \bool \\
\edg{\mathcal{I}}(x,y) & \defeq \tau(x)\to \tau(y) \\
\rfx{\mathcal{I}}(x) & \defeq \idfunc[\tau(x)]
\end{align*}
where $\tau:\bool\to\UU$ is the \define{tautological family} on the type $\bool$ of booleans, given by $\tau(\bfalse)\defeq\emptyt$ and $\tau(\btrue)\defeq\unit$. We write $a$ for the (unique) edge from $\bfalse$ to $\btrue$. It may be helpful to think of the walking edge $\mathcal{I}$ as an interval, hence the choice of notation.
\end{enumerate}
\end{eg}
...
\begin{defn}
Consider a reflexive graph $\mathcal{A}$ and a type $X$ equipped with a morphism $f:\mathsf{rGph}(\mathcal{A},\Delta(X))$. We say that $f$ is a \define{reflexive coequalizer of $\mathcal{A}$} if the map
\begin{equation*}
...
A morphism $f:\mathsf{rGph}(\mathcal{A},\mathcal{B})$ is said to be a \define{left fibration} of reflexive graphs if it is right orthogonal to the morphism $0:\mathsf{rGph}(\unit,\mathcal{I})$. Similarly, we say that $f$ is a \define{right fibration} of reflexive graphs if it is right orthogonal to the morphism $1:\mathsf{rGph}(\unit,\mathcal{I})$, and we say that $f$ is a \define{fibration} if it is both a left and a right fibration.
\end{defn}
...
\begin{comment}
\begin{cor}\label{cor:fibration_pullback}
Any pullback of a morphism between discrete reflexive graphs is a fibration.
\end{cor}

We will see in \cref{thm:etale_fibration} that a morphism of reflexive graphs is a fibration if and only if the square
\begin{equation*}
\begin{tikzcd}
...
\end{tikzcd}
\end{equation*}
of morphisms of reflexive graphs.
...
\begin{proof}
The type of identifications witnessing that the square commutes is equivalent to the type of triples $(H,K,L)$ consisting of
\begin{align*}
H : h_0\circ f_0\htpy g\circ k_0 \\
K : 
\end{align*}
\end{proof}
\end{comment}
...
\begin{rmk}
Using the walking edge $\mathcal{I}$ it can be shown that if the operation $\mathsf{equifib\usc{}fam}(\mathcal{I})$ is an equivalence, then the univalence axiom holds \cite{RijkeSpittersBarcelona}. Thus: `an interval satisfying descent implies univalence'.
\end{rmk}
...
\begin{comment}
\section{\texorpdfstring{$2$}{2}-simplicial types}
...
This question can by answered by studying `$2$-truncated simplicial types', by which we understand triples $(A_0,A_1,A_2)$ consisting of
\begin{align*}
A_0 & : \UU \\
A_1 & : A_0 \to A_0 \to \UU\\
A_2 & : \prd{x,y,z:A_0} A_1(x,y) \to A_1(x,z)\to A_1(y,z) \to \UU,
\end{align*}
representing the types of $0$-, $1$-, and $2$-cells, equipped with a `unital' or `simplicial' structure consisting of a unit (the reflexivity term of $A_1$), a right and left unit law and a coherence between them. In this section I give a brief sketch of some developments in this direction. Further details will appear in \cite{Rijke_simplicial}.
...
\begin{prp}\label{prp:nattrans_cartesian}
...
\begin{defn}
\begin{enumerate}
\item A \define{(sequential) cocone} on a type sequence $\mathcal{A}$ with vertex $X$ consists of
\begin{align*}
h & : \prd{n:\N} A_n\to X \\
...
\item We say that a cone $(h,H)$ with vertex $X$ is \define{colimiting} if the map $\mathsf{cocone\usc{}map}(h,H)$ is an equivalence for any type $Y$. 
\end{enumerate}
\end{defn}
...
\begin{lem}\label{thm:sequential_up}
