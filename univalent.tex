...
\item \cref{thm:pb_fibequiv}\marginnote{is this reference right?}, in which we establish that for any fiberwise map
\begin{equation*}
...
\marginnote{Make sure $\evpair$ is introduced, and shown to be an equivalence}
...
\end{equation*}
We also use pattern-matching notation $\lam{\,\refl{a}}p$ for $\ind{a=}(p)$.\marginnote{do we use this?}
...
\begin{eg}
\marginnote{Find a better place for this}The induction principles for $\Sigma$-types, $\emptyt$, $\unit$, and $\bool$ provide sections for the functions
\begin{align*}
...
We show in this section that a map $f:A\to B$ is an equivalence if and only if for any type family $P$ over $B$, the precomposition map\marginnote{I do not find this result in \cite{hottbook}}
\begin{equation*}
...
\begin{prp}
Let $\alpha:I\to J$ be a map, and let $P:I\to\UU$ and $Q:J\to\UU$ be type families.
Then the map
\begin{equation*}
\Big(\prd{i:I}P_i\to Q_{\alpha(i)}\Big) \eqvsym \mathsf{hom}_\alpha(\proj 1^P,\proj 1^Q). 
\end{equation*}
given by $\lam{f}(\total[i]{f},\lam{(x,y)}\refl{x})$ is an equivalence.
\end{prp}
...
The following corollary is also called the \define{pasting property} of pullbacks.\index{pasting property!of pullbacks|textit}
...
\begin{prp}
Consider a commuting square\marginnote{move to where first needed?}
\begin{equation*}
\begin{tikzcd}
A \arrow[d,swap,"f"] \arrow[r,"i"] & B \arrow[d,"g"] \\
X \arrow[r,swap,"j"] & Y
...
The following are equivalent:\marginnote{Do we need these?}
\begin{enumerate}
