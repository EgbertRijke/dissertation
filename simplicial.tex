...
The \define{standard $1$-simplex} $\Delta[1]$ is the $2$-simplicial type with two vertices $0$ and $1$, and a non-degenerate edge $s$ from $0$ to $1$. The universal property of the standard $1$-simplex is that the function
\begin{equation*}
...
\end{equation*}
A morphism $f:\mathsf{sType}_2(\mathcal{A},\Delta X)$ is said to be a \define{typal realization}\marginnote{If typal realization is not good terminology, could use `$2$-simplicial coequalizer' or something along those lines} of $\mathcal{A}$ if the function
\begin{equation*}
...
\section{typal realizations as iterated pushouts}
